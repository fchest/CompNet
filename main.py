...
import warnings
import toml
from utils.util import json_extraction, numParams, logger_print
...
def main(config):
    ...
    save_opt = config["save"]["save_opt"]

    # set logger
    if not os.path.exists(config["path"]["logging_path"]):
        os.makedirs(config["path"]["logging_path"])
    logging.basicConfig(filename=config["path"]["logging_path"] + "/" + config["save"][save_opt]["paths"]["logger_filename"],
                        filemode='w',
                        level=logging.INFO,
                        format="%(message)s")
    ...
    # save filename
    save_name_dict = {}
    save_name_dict["loss_filename"] = config["save"][save_opt]["paths"]["loss_filename"]
    save_name_dict["best_model_filename"] = config["save"][save_opt]["paths"]["best_model_filename"]
    save_name_dict["checkpoint_filename"] = config["save"][save_opt]["paths"]["checkpoint_filename"]

    # determine file json
    train_mix_json = json_extraction(config["path"]["train"]["mix_file_path"], config["dataset"]["train"]["json_path"],
                                     "mix")
